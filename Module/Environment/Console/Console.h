...
std::string GetInput() {
    ...
    if (hInput == INVALID_HANDLE_VALUE) {
        MessageBoxA(0, "Could not get handle!", "Blackline", 0);
        return "";
    }
    ...
    if (ReadConsole(hInput, buffer, sizeof(buffer) - 1, &bytesRead, NULL)) {
        ...
        MessageBoxA(0, "Error reading console!", "Blackline", 0);
        return "";
    }
...
namespace ConsoleLib {
    int rconsolecreate(lua_State* L) {
        ...
        if (!result) {
            ...
            MessageBox(0, "You're about to get Cooked", "You're about to get Cooked", 0);
        }
