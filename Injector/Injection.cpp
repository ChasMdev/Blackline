...
int InjectionDllCaller() {
	...
	auto WhitelistPage = [](uintptr_t page) {
		...
		auto SetInsert = (TSetInsert)(hyperionBase + Offsets::offset_SetInsert);
		void* whitelistedPages = (void*)(hyperionBase + Offsets::offset_WhitelistedPages);
		...
		*reinterpret_cast<uint32_t*>(&_) = (Page >> 0xc) ^ Offsets::pageHash;
		*reinterpret_cast<uint8_t*>(&_.Byte4) = ((Page >> 0x2c) & 0xFF) ^ Offsets::byteHash;


		SetInsert(whitelistedPages, &Null, &_);

		uintptr_t bitmap = *(uintptr_t*)(hyperionBase + Offsets::offset_Bitmap);
